...
reg_t illegal_instruction(processor_t UNUSED *p, const insn_t& insn, const reg_t UNUSED& pc)
{
  // The illegal instruction can be longer than ILEN bits, where the tval will
  // contain the first ILEN bits of the faulting instruction. We hard-code the
  // ILEN to 32 bits since all official instructions have at most 32 bits.
...
#else // SPIKE_ABSTRACT_PROCESSOR
#define DEFINE_INSN(name) \
    extern reg_t fast_rv32i_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t fast_rv64i_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t fast_rv32e_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t fast_rv64e_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t logged_rv32i_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t logged_rv64i_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t logged_rv32e_##name(processor_t*, const insn_t&, const reg_t&); \
    extern reg_t logged_rv64e_##name(processor_t*, const insn_t&, const reg_t&); \
    if (name##_supported) { \
      register_insn((insn_desc_t) { \
        name##_match, \
        name##_mask, \
